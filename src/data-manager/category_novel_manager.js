...
class NovelByCategoryManager extends DataManagementInterface {
  ...
  get(key) {
    switch (key) {
      ...
      default: {
        console.log(
          `Cannot find property ${key} in searching novel by category manager!`
        );
        return null;
      }
    }
  ...
  async loadSource() {
    ...
    for (let i in sources) {
      let source_meta = await getNovelByCategory(
        sources[i].slug,
        this.category,
        1
      );
      if (source_meta.meta) {
        this.sources.push({
          slug: sources[i].slug,
          total_page: source_meta.meta.total_pages,
        });
      }
    }

    let result = [
      ...new Map(this.sources.map((item) => [item["slug"], item])).values(),
    ];
    this.sources = [...result];
    this.total_page = this.sources
      .map((item) => item.total_page)
  ...
  async queryNovels() {
    ...
    let novels = await getNovelByCategory(
      query_source,
      this.category,
      query_page
    );
